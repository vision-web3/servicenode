...
class HubContract(Base):
    ...
    represents a Vision Hub contract deployed on a blockchain. It can be
    either the current or an old version of the Vision Hub contract on
    its blockchain.
...
class Bid(Base):
    ...
    represents a service node bid registered at the Vision Hub of a
...
class Transfer(Base):
    ...
    represents a (possibly cross-chain) Vision token transfer.
    ...
    forwarder_contract_id : sqlalchemy.Column
        The unique ID of the Vision Forwarder contract on the source
        blockchain (foreign key).
